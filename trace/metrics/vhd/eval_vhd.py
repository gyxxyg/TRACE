...
def compute_hl_hit1(qid2preds, qid2gt_scores_binary):
    ...
    for idx, qid in enumerate(qids):
        ...
        # print(qid2preds[qid], pred_clip_idx, gt_scores_binary[pred_clip_idx])
        # for potential_clip_idx in range(len(qid2preds[qid]['pred_saliency_scores'])):
        #     if qid2preds[qid]['pred_saliency_scores'][potential_clip_idx] == qid2preds[qid]['pred_saliency_scores'][pred_clip_idx]:
        #         if np.max(hit_scores[idx]) < np.max(gt_scores_binary[potential_clip_idx]):
        #             hit_scores[idx] = gt_scores_binary[potential_clip_idx]
